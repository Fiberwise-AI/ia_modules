...
class RedisConversationMemory(ConversationMemory):
    ...
    async def add_message(
        ...
        await self.redis.setex(message_key, self.ttl, json.dumps(message_data))

        # Add to thread sorted set
        thread_key = f"thread:{thread_id}"
        await self.redis.zadd(thread_key, {message_id: timestamp.timestamp()})
        await self.redis.expire(thread_key, self.ttl)

        # Add to user index if user_id provided
        if user_id:
            user_key = f"user:{user_id}:messages"
            await self.redis.zadd(user_key, {message_id: timestamp.timestamp()})
            await self.redis.expire(user_key, self.ttl)

            # Store thread->message mapping for user
            user_thread_key = f"user:{user_id}:thread:{thread_id}"
            await self.redis.sadd(user_thread_key, message_id)
            await self.redis.expire(user_thread_key, self.ttl)
    ...
    async def get_messages(
        ...
        # Get message IDs from sorted set (reverse order - newest first)
